 ...
 */
export const getSearchConsoleVerification = async (siteId: string): Promise<SearchConsoleVerification> => {
  try {
    const response = await apiClient.get(`/seo/search-console/${siteId}`);
    return response.data;
  } catch (error) {
    console.error("서치 콘솔 확인 코드를 가져오는 중 오류가 발생했습니다:", error);
    throw error;
  }
};
...
export const saveSearchConsoleVerification = async (
  ...
  try {
    const response = await apiClient.post(`/seo/search-console/${siteId}`, data);
    return response.data;
  } catch (error) {
    console.error("서치 콘솔 확인 코드를 저장하는 중 오류가 발생했습니다:", error);
    throw error;
  }
};
